data{
    ...
    vector[N_obs] x_3; // Average score for Rule of law 2013-21
...
transformed parameters {
    ...
    vector[N_obs] mu;
    for (i in 1:N_obs) {
        mu[i] = a_1[Rid[i]] + a_2 * x_2[i] + x_1[i] * (b_1 + b_2 * x_2[i]) + x_3[i] * (b_3 + b_4 * x_2[i]);
    }
}
...
model{
    ...
    // prior
    target += uniform_lpdf(sigma_a_1 | 0, 10); 
    target += uniform_lpdf(sigma_y | 0, 10);
    
    target += normal_lpdf(mu_a_1 | 0, 10);
